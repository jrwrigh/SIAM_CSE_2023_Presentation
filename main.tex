...
\begin{frame}{What is libCEED?}
    ...
    \begin{itemize}
        ...
        \item<+-> Performance demonstrated for solids.......
    \end{itemize}
    ...
    \onslide<2->{
        \begin{tikzpicture}
            ...
            \begin{scope}[
                ...
                \invisible<6->{\draw[ultra thin,white,fill=white] (10, 0) rectangle (0, 2);}
            \end{scope}
...
\begin{frame}{Exact Matrix-Free Jacobian via CeedOperator}
    ...
    \onslide<4->{
        \begin{itemize}
            \item \textbf{Pros:} Exact Jacobian matrix-vector product\footnote<5->{Affect of specific terms may be ignored from the Jacobian. This is done for \(\derd\bm{\tau}/\derd\bm{Y}\)} (potentially faster convergence)
    ...
    \begin{columns}
        ...
        \onslide<+->{
            \begin{figure}
                ...
                \vspace{-30pt}
                \caption{\tiny Run on ALCF's Polaris on two nodes (8\(\times\) NVIDIA A100, global DoF count fixed}
        ...
        \begin{itemize}
            \item<+-> Time of \(\derd \mathcal{G} / \derd \bm{Y} \Delta \bm{Y}\) decreases as order increases
                \begin{itemize}
                    \item Despite increase in \(\derd \mathcal{G} / \derd \bm{Y} \Delta \bm{Y}\) calculations per timestep:
                        \begin{table}
                            \begin{tabular}{ccl}
                                \(Q_1\) & \(Q_2\) & \(Q_3\) \\
                                \toprule
                                % \midrule
                                \textcolor{red}{250} & 221 & 360 \\
                                \bottomrule
                            \end{tabular}
                        \end{table}
