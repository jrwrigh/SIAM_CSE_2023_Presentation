...
\section{Accuracy and Performance of High-Order Scale-Resolving Simulations}
...
\begin{frame}{Viscous Outflow Boundary Conditions}
    \begin{itemize}
        \item<2-> Riemann Flux BCs everywhere is ideal
            ...
            % \end{itemize}
        % \item<5-> Solution: Calculate \(\bm{F}_i^{\text{adv}}\) via Riemann solver, calculate \(\bm{F}_i^{\text{diff}}\) from solution
        %     \begin{itemize}
        %         \item<5-> Handles recirculation without issue, velocity need not be set
        %     \end{itemize}
    \end{itemize}
    \begin{block}{Solution}<5->
        Calculate \(\bm{F}_i^{\text{adv}}\) via Riemann solver, calculate \(\bm{F}_i^{\text{diff}}\) from solution
        \vspace{-10pt}
        \begin{itemize}
            \item Handles recirculation without issue, velocity need not be set
        \end{itemize}
    \end{block}
        \vspace{-15pt}
    \begin{columns}
    ...
    \onslide<2->{\begin{figure}
        \begin{tikzpicture}
                \node [
                above right,
                inner sep=0] (image) at (0,0) {\includegraphics[width=0.35\textwidth]{img/RiemannFreestream.pdf}};
            ...
            \begin{scope}[
                ...
                \onslide<6->{\draw[red] (0, 4.5) rectangle (10, 2.5);}
            \end{scope}
        \end{tikzpicture}
        \vspace{-10pt}
    ...
    \onslide<5->{ \begin{figure}
        \begin{tikzpicture}
            ...
            \begin{scope}[
                ...
                \onslide<6->{\draw[red] (0, 4.3) rectangle (9.2, 2.3);}
            \end{scope}
        \end{tikzpicture}
        \vspace{-10pt}
...
\begin{frame}{Support and References}
    ...
    \textcolor{red}{Add in sponsor support (DOE, ECP, etc)}
\end{frame}
