...
\begin{frame}{Implicit Timestepping}
    ...
    \begin{equation*}
        \diff{\mathcal{G}(\bm{U}_{,t}, \bm{U})}{\bm{U}} \Delta \bm{U} = \mathcal{G}(\bm{U}_{,t}, \bm{U})
    \end{equation*}
    \vspace{-30pt}
    \begin{itemize}
        \item<2-> Store \(\frac{\mathrm{d} \mathcal{G}}{\mathrm{d}\bm{U}}\)directly
            \begin{itemize}
        ...
        \item<3-> Finite difference matrix-free approximation:
            \begin{equation*}
                \diff{\mathcal{G}(\bm{U}_{,t}, \bm{U})}{\bm{U}} \Delta \bm{U} \approx \frac{\mathcal{G}(\bm{U}_{,t}, \bm{U} + \epsilon \Delta \bm{U}) - \mathcal{G}(\bm{U}_{,t}, \bm{U})} {\epsilon}
            \end{equation*}
            \vspace{-20pt}
            \begin{itemize}
...
\begin{frame}{Exact Matrix-Free Jacobian}
    \begin{align*}
        \onslide<+->{\diff{\mathcal{G}}{\bm{U}} \Delta \bm{U} &= \diff{}{\bm{U}} \left[\mathcal{G}(\bm{U}_{,t}, \bm{U})\right] \Delta \bm{U} \\}
        \onslide<+->{ &= \diff{}{\bm{U}} \left[ \PartT \ElemT \BasT \Qfunc{G} \Bas \Elem \Part \right] \Delta \bm{U} \\}
        \onslide<+->{ &= \left[ \PartT \ElemT \BasT \diff{\Qfunc{G}}{\bm{U}}  \Bas \Elem \Part \right]\Delta \bm{U} \\}
    \end{align*}
    \vspace{-50pt}
    \onslide<+->{
    \begin{itemize}
        \item Store intermediary data at quadrature points to improve efficiency ("taping")
            \begin{itemize}
                \item We store \(\bm{U}\), viscous stress, and stabilization perturbation (\(\mathcal{P}(\bm{v})\))
            \end{itemize}
