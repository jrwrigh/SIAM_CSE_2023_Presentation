...
\begin{frame}{Compressible Navier-Stokes for Continuous-Galerkin FEM}
    ...
    \begin{equation*}
        \begin{aligned}
            \int_{\Omega} \bm v \cdot \left( \bm{A_0}\bm{Y}_{,t} - \bm{S}(\bm{Y}) \right)  \,\dif \Omega
            ...
            \onslide<2->{\underbrace{+ \int_{\Omega} P(\bm v)^T \, \left(\bm{A_0}\bm{Y}_{,t} + \bm{F}_{i,i}(\bm{Y}) - S(\bm{Y})
    ...
    \onslide<3->{
        ...
        \begin{align*}
            ...
            \onslide<4->{\Rightarrow \quad \PartT \ElemT \BasT \Qfunc{G} \Bas \Elem \Part
                \begin{bmatrix}
                    \bm{Y}_{,t} \\
                    \bm{Y}
                \end{bmatrix} &= 0}
...
\begin{frame}{Viscous Outflow Boundary Conditions}
    \begin{itemize}
        ...
        \item<4-> Weak-pressure prescription ill-posed for recirculation
    \end{itemize}
    \begin{block}{Solution}<5->
        \vspace{-5pt}
        \begin{itemize}
            \item<5-> Calculate \(\bm{F}_i^{\text{adv}}\) via Riemann solver, setting \(u_i^{\text{ext}} = u_i^{\text{int}}\) 
            \item<7-> Calculate \(\bm{F}_i^{\text{diff}}\) from solution, then \(\bm{F}_i = \bm{F}_i^{\text{adv}} + \bm{F}_i^{\text{diff}}\)
        \end{itemize}
    \end{block}
        \vspace{-15pt}
    \begin{columns}
    ...
    \onslide<2->{ \begin{figure}
        \begin{tikzpicture}
            ...
            \begin{scope}[
                ...
                \onslide<6->{\draw[red, thick] (0, 4.3) rectangle (8, 2.3);}
            \end{scope}
        \end{tikzpicture}
        \vspace{-10pt}
    ...
    \onslide<4>{
        \begin{tikzpicture}[remember picture,overlay]
            \node[above left, xshift=-0cm,yshift=1cm] (image) at (current page.south east) {\includegraphics[width=8cm]{img/FlatPlateRecirculation.png}};
        \end{tikzpicture}
    }
    \onslide<5->{ \begin{figure}
        \begin{tikzpicture}
            ...
            \begin{scope}[
                ...
                \onslide<6->{\draw[red, thick] (0, 4.3) rectangle (9.2, 2.3);}
            \end{scope}
        \end{tikzpicture}
        \vspace{-10pt}
    ...
    \begin{columns}
        ...
        \column{0.5\textwidth}
            \begin{itemize}
                ...
                \item Stable for significant outflow recirculation
    ...
    \begin{itemize}
        ...
        \item<3-> Finite difference matrix-free approximation:
            ...
            \begin{itemize}
                ...
                \item \textbf{Cons:} Approximation, accuracy limited to \(\sqrt{\epsilon_{\text{machine}}}\)
            \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Exact Matrix-Free Jacobian via CeedOperator}
    \begin{columns}
        ...
        \onslide<1->{
            \begin{align*}
                \diff{\mathcal{G}}{\bm{Y}} \Delta \bm{Y} &= \diff{}{\bm{Y}} \left[ \PartT \ElemT \BasT \Qfunc{G} \Bas \Elem \Part \right] \Delta \bm{Y} \\
            ...
            \begin{center}
                \begin{tikzpicture}
                    ...
                    \begin{scope}[
                        ...
                        \invisible<3->{\draw[ultra thin,white,fill=white] (6.7, 0) rectangle (0, 10);}
                    \end{scope}
                \end{tikzpicture}
            \end{center}
        }
    ...
    \onslide<4->{
        \begin{itemize}
            \item \textbf{Pros:} Exact Jacobian matrix-vector product (potentially faster convergence)
            \item \textbf{Cons:} More expensive than residual evaluation (but not by too much)
            \begin{itemize}
                \item Still faster than just \textit{applying} sparse \(\frac{\mathrm{d} \mathcal{G}}{\mathrm{d}\bm{Y}}\)
            \end{itemize}
        \end{itemize}
    }
\end{frame}
...
\begin{frame}{Performance Exact Matrix-Free Jacobian via CeedOperator}
    \begin{columns}
        ...
        \onslide<+->{
            \begin{figure}
                \centering
                \includegraphics[width=0.95\textwidth]{img/OpAppEfficiency_PerfPortSolidsFig7.png}
                \vspace{-10pt}
        ...
        \onslide<+->{
            \begin{figure}
                \centering
                \includegraphics[width=0.95\textwidth]{img/PerformanceBarChart.pdf}
                \vspace{-10pt}
            ...
            %         Order & Time per Step \\
            %         \midrule
            %         \(Q_1\) & 0.910\unit{\second} \\
            %         \(Q_2\) & 0.631\unit{\second} \\
            %         \(Q_3\) & 0.505\unit{\second} \\
            %         \bottomrule
            %     \end{tabular}
                % \caption{\tiny Fluids performance, DoFs fixed.\\ Run on ALCF's Polaris on two nodes (8\(\times\) NVIDIA A100)}
                % % From looking at logs in /eagle/cfdml_aesp/PolarisMilestone/Cases/8part{,P2,P3}
