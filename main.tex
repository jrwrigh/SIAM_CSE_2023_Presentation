...
\begin{frame}{Compressible Navier-Stokes for FEM}
    ...
    \onslide<3->{
        ...
        \begin{align*}
            ...
            \onslide<4->{\Rightarrow \quad \left[\PartT \ElemT \BasT \Qfunc{G} \Bas \Elem \Part \right]
                \begin{bmatrix}
                    \bm{Y}_{,t} \\
                    \bm{Y}
                \end{bmatrix} &= 0}
...
\begin{frame}{Implicit Timestepping}
    ...
    \vspace{-30pt}
    \begin{itemize}
        \item<2-> Store \(\frac{\mathrm{d} \mathcal{G}}{\mathrm{d}\bm{Y}}\)directly
            \begin{itemize}
                \item Pros: Opens up preconditioning options
                \item Cons: Is large, expensive to store
            \end{itemize}
        \item<3-> Finite difference matrix-free approximation:
            ...
            \vspace{-20pt}
            \begin{itemize}
                \item Pros: Just need a residual evaluation, cheap (in programming and computation)
                \item Cons: Approximation, accuracy limited to \(\sqrt{\epsilon_{\text{machine}}}\)
            \end{itemize}
    \end{itemize}
\end{frame}

\begin{frame}{Exact Matrix-Free Jacobian}
    \begin{align*}
        \onslide<+->{\diff{\mathcal{G}}{\bm{Y}} \Delta \bm{Y} &= \diff{}{\bm{Y}} \left[\mathcal{G}(\bm{Y}_{,t}, \bm{Y})\right] \Delta \bm{Y} \\}
        \onslide<+->{ &= \diff{}{\bm{Y}} \left[ \PartT \ElemT \BasT \Qfunc{G} \Bas \Elem \Part \right] \Delta \bm{Y} \\}
        \onslide<+->{ &= \left[ \PartT \ElemT \BasT \diff{\Qfunc{G}}{\bm{Y}}  \Bas \Elem \Part \right]\Delta \bm{Y} \\}
    \end{align*}
    \vspace{-50pt}
    \onslide<+->{
    \begin{itemize}
        \item Store intermediary data at quadrature points to improve efficiency ("taping")
            \begin{itemize}
                \item We store \(\bm{Y}\), viscous stress, and stabilization perturbation (\(P(\bm{v})\))
            \end{itemize}
        \item Pros: Exact Jacobian matrix-vector product (potentially faster convergence)
        \item Cons: More expensive than residual evaluation (but not by too much)
    \end{itemize}
    }
\end{frame}
    ...
    \begin{itemize}
        ...
        \item<+-> libCEED Operator based on user-implemented \texttt{CeedQFunction}s (\(\DQfunc\))
            \begin{itemize}
                ...
                \item Combined into a single \texttt{CeedOperator} to represent \(\mathcal{G}(\bm{Y}_{,t}, \bm{Y})\)
            \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Time Step Loop}
    \begin{enumerate}
        \item<+-> PETSc gets \(\bm{Y}^L = \Part \bm{Y}^G\) from current solution
        \item<+-> PETSc calls libCEED to get \(\bm{G}^L = \underbrace{\ElemT \BasT \DQfunc \Bas \Elem}_L \bm{Y}^L\)
