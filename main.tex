...
\begin{frame}{Compressible Navier-Stokes for Continuous-Galerkin FEM}
    ...
    \begin{equation*}
        \begin{aligned}
            \only<-1>{\quad\ \ \,}\int_{\Omega} \bm v \cdot \left( \bm{A_0}\bm{Y}_{,t} - \bm{S}(\bm{Y}) \right)  \,\dif \Omega
            \only<-1>{
            ...
            \onslide<3->{\underbrace{+ \int_{\Omega} P(\bm v)^T \, \left(\bm{A_0}\bm{Y}_{,t} + \bm{F}_{i,i}(\bm{Y}) - S(\bm{Y})
...
\begin{frame}{Exact Matrix-Free Jacobian via CeedOperator}
    ...
    \onslide<4->{
        \begin{itemize}
            ...
            \item \textbf{Cons:} Preconditioning requires partial assembly
        \end{itemize}
    }
\end{frame}
